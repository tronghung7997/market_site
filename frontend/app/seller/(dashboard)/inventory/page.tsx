...
export default function InventoryPage() {
  ...
  return (
    <Suspense fallback={<Spinner label="Đang tải kho hàng" />}>
      <Inventory />
    </Suspense>
  );
}
...
function Inventory() {
  ...
  if (loading) return <Spinner label="Đang tải kho hàng" />;
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-end justify-between gap-4 flex-wrap">
        <div>
          <h1 className="font-serif text-[24px] font-semibold tracking-tight">Kho hàng</h1>
          <p className="text-[12.5px] text-muted mt-0.5">
            <span className="font-mono tabular text-good font-medium">{totalAvailable}</span> sản phẩm
            sẵn sàng bán. Sửa nội dung ngay trên dòng — dòng đã giao cho khách thì khoá lại.
          </p>
        </div>
      ...
      {byProduct.length === 0 ? (
        <Card className="p-8 text-center">
          <p className="text-[13px] text-muted">
            {rows.length === 0
              ? "Chưa có gói sản phẩm nào. Tạo sản phẩm và gói trước, rồi quay lại đây nạp hàng."
              : "Không có gói nào khớp bộ lọc này."}
          </p>
        </Card>
      ) : (
        <div className="space-y-5">
      ...
      >
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-center gap-2">
            ...
            {out && <Tag tone="bad">Hết hàng</Tag>}
            {low && <Tag tone="warn">Sắp hết</Tag>}
            {!row.is_active && <Tag tone="neutral">Đang tắt</Tag>}
          </div>
        ...
        <div className="shrink-0 text-right font-mono text-[12px] tabular">
          ...
          <div className="text-faint">{row.assigned} đã bán</div>
        </div>
      </button>
...
function VariantLines({ variantId, onChanged }: { variantId: number; onChanged: () => void }) {
  ...
  if (items === null) {
    return <div className="px-5 pb-4 pl-12 text-[12px] text-faint">Đang tải…</div>;
  }
    ...
    <div className="pl-12 pr-5 pb-4 space-y-1.5 bg-raised/30">
      {items.length === 0 && !adding && (
        <p className="py-2 text-[12.5px] text-muted">
          Gói này chưa có hàng. Dán danh sách vào để mở bán.
        </p>
      )}
      ...
      >
        {editing ? (
          ...
          <button
            ...
            disabled={!editable}
            title={editable ? "Bấm để sửa" : undefined}
            className={cn(
