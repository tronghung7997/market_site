...
export default function SellerLayout({ children }: { children: ReactNode }) {
  ...
  if (loading) {
    return (
      <div className="w-full mx-auto max-w-[1200px] px-6 py-6 animate-pulse">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            <span className="h-9 w-9 rounded-lg bg-raised border border-line" />
            <div className="space-y-1.5">
              <div className="h-4.5 w-32 rounded bg-raised" />
              <div className="h-3 w-24 rounded bg-raised" />
            </div>
          </div>
        </div>
        <div className="mb-6 border-b border-line pb-px">
          <div className="flex gap-4">
            {[1, 2, 3, 4, 5, 6].map((i) => (
              <div key={i} className="h-7 w-20 rounded bg-raised mb-2" />
            ))}
          </div>
        </div>
        <div className="space-y-4">
          ...
          <div className="h-64 rounded-xl bg-raised border border-line" />
        </div>
      </div>
    );
  }
