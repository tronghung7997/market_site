...
`products` owns admin moderation of any seller's product. `PATCH /admin/products/{id}` accepts only the four lifecycle statuses; `POST /admin/products/bulk` (≤100 ids, one transaction) activates, pauses, drafts, suspends or re-categorises and reports skipped ids (`not_found`, `unchanged`, `needs_setup` — bulk activation never puts an unconfigured product on sale); `POST /admin/products/{id}/suspend` takes an optional reason. Every admin status, category and content change (including `PATCH /admin/products/{id}/translations/{locale}`, which logs only fields whose value changed) is a `log_entries` row with `subject_type=product`, read back by `GET /admin/products/{id}/activity`. A product that is not `active` answers 404 on the public `GET /products/{ref}`; its owner previews the buyer page through `GET /seller/products/{ref}/preview` (another seller's product is 404) and admins through `GET /admin/products/{ref}/preview` — same public-shaped payload, and order creation still rejects non-active products. `GET /admin/products` rows carry the storefront `price_from`, sellable `stock_count` (null when the product is not inventory-managed), `variant_count` and category, computed for the current page only.

`suppliers` owns upstream sources managed from `/admin/sources` (and `/seller/sources`, internal sellers only — `accounts.is_internal` is checked by the router, not just hidden in the UI). Catalog sources (`AdapterSpec.external_stock`, igbm) sell through `supplier_listings`; proxy sources (`AdapterSpec.proxy_source`, DProxy/TopProxy) sync a plan catalog into `supplier_catalog_items` and sell through config-pricing `plan_prices` (`suppliers/proxy_sources.py`). DProxy's provider-wide `plan_ids` matrix is written only by `_merge_plan_ids` (row lock, 409 on a conflicting key, audit). Every price save of a proxy-source product passes `enforce_offer_margins` (cost from the synced catalog, USD converted at the display rate; TopProxy cost from the `adapters/topproxy_costs.py` ladder, and a TopProxy plan with no cost or an unknown `loaiproxy`/protocol is rejected as `PROXY_PLAN_UNSUPPORTED`). The seller edits those prices per plan on the product page: `GET /products/{id}/proxy-plans` (owner or admin) returns every plan with cost, floor and margin — a legacy formula product is expanded to its equivalent plan table — and `POST /products/{id}/proxy-plans/quote` prices a draft plan without writing; the save itself is the ordinary `PUT /seller/products/{id}/pricing` with `plan_prices`. Providers of non-seller-registrable adapters owned by an internal seller keep admin trust (`adapters/factory.py::_seller_owned`) and cannot be edited from the seller side. Upstream revocations (DProxy `partner-dispute`) are never sent inside a refund transaction: `resources/proxy_service.enqueue_upstream_revocation` writes an `upstream_revocations` row in the caller's transaction and `scheduler.upstream_revocation_job` sends it after commit with backoff, raising `upstream_revoke_failed` when it gives up. New critical incidents are also mailed to active admins (`alerts.service._mail_admins`, template `ops_incident`).
