    ...
    <div className="space-y-5">
      ...
      {dynamic && (
        <div id="product-integration" className="scroll-mt-24 space-y-4 rounded-xl border border-line bg-surface p-4">
          ...
          <Field label={tf("integrationLabel")} hint={core.compatibleProviders.length > 0 ? tf("integrationHint") : tf("noCompatibleIntegrations")}>
            <Select value={core.selectedProviderId} onChange={(e) => core.setSelectedProviderId(Number(e.target.value))} disabled={core.compatibleProviders.length === 0}>
              {core.compatibleProviders.length === 0 && <option value={0}>{tf("integrationPlaceholder")}</option>}
              {core.compatibleProviders.map((provider) => <option key={provider.id} value={provider.id}>{provider.name}</option>)}
            </Select>
          </Field>
          {core.compatibleProviders.length === 0 && (
            <Button size="sm" variant="secondary" onClick={() => router.push("/seller/providers")}>{tf("createCompatibleIntegration")}</Button>
          )}
          {core.workModel === "B1" && <Field label={tf("monthlyBasePrice", { currency })}><SellerPriceInput amountVnd={core.b1.basePrice} onAmountVndChange={(basePrice) => core.setB1({ ...core.b1, basePrice })} /></Field>}
