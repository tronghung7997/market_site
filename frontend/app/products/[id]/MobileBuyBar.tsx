 ...
 *  (cha quyết định qua `visible`), bấm là cuộn về phiếu. Thuần hiển thị. */
...
export default function MobileBuyBar({ visible, total, fallbackText, onGoToPanel }: {
  visible: boolean;
  /** null = không có tổng để khoe (form động / gói liên hệ) → hiện fallbackText. */
  total: number | null;
  fallbackText: string;
  onGoToPanel: () => void;
}) {
  return (
    ...
    >
      ...
      <Button tabIndex={visible ? 0 : -1} onClick={onGoToPanel}>Đặt hàng</Button>
    </div>
  );
}
