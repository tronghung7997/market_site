  ...
  return (
    <div className="space-y-7">
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          <h2 className="text-[16px] font-semibold">{t("orders")}</h2>
          <p className="text-[13px] text-muted mt-1">
            {t("orderCountShort", { count: orders.length })}
            {needsAction > 0 && <> · <span className="text-warn font-medium">{t("needsAction", { count: needsAction })}</span></>}
          </p>
        </div>
        <Link href="/seller" className="text-[12.5px] text-iris-hi hover:underline flex items-center gap-1 shrink-0">
          {t("viewRevenue")} <ArrowRight size={12} />
        </Link>
      </div>
      ...
      {orders.length === 0 && (
        <Card className="p-8 text-center">
          ...
          <p className="text-[14px] text-muted">{t("noOrders")}</p>
        </Card>
      )}

      {disputesUnresponded.length > 0 && (
        <Section title={t("disputesNeedResponse", { count: disputesUnresponded.length })} icon={<Info size={14} />} tone="bad">
      ...
      {pending.length > 0 && (
        <Section title={t("newOrdersNeedAcceptance", { count: pending.length })} icon={<Clock size={14} />} tone="warn">
          {pending.map((o) => (
      ...
      {processing.length > 0 && (
        <Section title={t("preparingDelivery", { count: processing.length })} icon={<Clock size={14} />} tone="iris">
          {processing.map((o) => (
            <SellerOrderCard key={o.id} order={o} acting={acting}
              onAccept={accept} onDeliver={deliver} onDisputeResponded={load} />
          ))}
      ...
      {disputesResponded.length > 0 && (
        <Section title={t("disputesWaitingAdmin", { count: disputesResponded.length })} icon={<Info size={14} />} tone="neutral">
      ...
      {history.length > 0 && (
        <div>
          <h3 className="text-[13px] font-semibold text-muted mb-3">{t("history", { count: history.length })}</h3>

          <div className="flex items-center gap-2 mb-3 flex-wrap">
            <div className="flex gap-1 flex-wrap">
              <FilterTab active={historyFilter === "all"} onClick={() => setHistoryFilter("all")}>
                {t("all", { count: history.length })}
              </FilterTab>
          ...
          <div className="space-y-2">
            ...
            {filteredHistory.length === 0 && (
              <p className="px-2 py-6 text-center text-[13px] text-muted">{t("noMatchingOrders")}</p>
            )}
          </div>
        </div>
      )}
