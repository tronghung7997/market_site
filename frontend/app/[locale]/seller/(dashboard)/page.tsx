...
function build7DayRevenue(orders: Order[], locale: string) {
  ...
  for (let i = 6; i >= 0; i--) {
    ...
    days.push({
      ...
      full: d.toLocaleDateString(locale, { day: "numeric", month: "short" }),
      value,
    });
  }
  return days;
...
export default function SellerDashboard() {
  ...
  const revenue = useMemo(() => build7DayRevenue(orders, locale), [orders, locale]);
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap justify-end gap-3">
        <Link href="/seller/products/new">
          <Button><Plus size={15} /> {t("newProduct")}</Button>
        </Link>
        <Link href="/seller/orders">
          <Button variant="secondary"><Inbox size={15} /> {t("viewOrders")}</Button>
        </Link>
        <Link href="/seller/products">
          <Button variant="secondary"><Package size={15} /> {t("manageProducts")}</Button>
        </Link>
      </div>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <Card className="p-5">
          <h3 className="text-[13px] font-semibold mb-4 flex items-center gap-2">
            <BarIcon size={14} className="text-faint" /> {t("revenue7Days")}
          </h3>
        ...
        <Card className="p-5">
          <h3 className="text-[13px] font-semibold mb-4 flex items-center gap-2">
            <Clock size={14} className="text-faint" /> {t("ordersByStatus")}
          </h3>
...
function OrderBreakdown({ pending, processing, delivered, completed, total }: {
  ...
  const t = useTranslations("seller");
  const items = [
    { label: t("pendingStatus"), count: pending, color: "var(--color-warn)" },
    { label: t("processingStatus"), count: processing, color: "var(--color-iris-hi)" },
    { label: t("deliveredStatus"), count: delivered, color: "var(--color-good)" },
    { label: t("completedStatus"), count: completed, color: "var(--color-good)" },
  ];
  ...
  return (
    <div className="space-y-4">
      {shown > 0 ? (
        ...
        <div className="text-[13px] text-muted">{t("noOrders")}</div>
      )}
