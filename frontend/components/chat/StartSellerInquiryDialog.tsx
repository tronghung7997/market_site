...
const copy = {
  vi: {
    action: "Nhắn tin", title: "Nhắn cho người bán", hint: "Chọn sản phẩm để người bán biết bạn đang cần hỗ trợ về nội dung nào.",
    product: "Sản phẩm", message: "Nội dung", placeholder: "Nhập câu hỏi của bạn...", cancel: "Huỷ",
    send: "Gửi tin nhắn", sending: "Đang gửi...", noProducts: "Gian hàng chưa có sản phẩm để trao đổi.",
  },
  en: {
    action: "Message", title: "Message seller", hint: "Choose a product so the seller knows what you need help with.",
    product: "Product", message: "Message", placeholder: "Write your question...", cancel: "Cancel",
    send: "Send message", sending: "Sending...", noProducts: "This shop has no products available for inquiry.",
  },
};
...
export default function StartSellerInquiryDialog({
  ...
  return (
    <>
      <Button type="button" size="sm" onClick={start} disabled={available.length === 0} title={available.length === 0 ? t.noProducts : t.action}>
        <MessageCircle size={14} /> {t.action}
      </Button>
      {open && (
        <div className="fixed inset-0 z-50 grid place-items-center px-4">
          ...
          <section role="dialog" aria-modal="true" aria-labelledby="seller-inquiry-title" className="relative w-full max-w-[500px] rounded-xl border border-line bg-surface shadow-card-lg">
            <header className="flex items-start justify-between gap-4 border-b border-line px-5 py-4">
              <div>
                <h2 id="seller-inquiry-title" className="text-[16px] font-semibold">{t.title}: {sellerName}</h2>
                <p className="mt-1 text-[12.5px] leading-relaxed text-muted">{t.hint}</p>
              </div>
            ...
            <div className="space-y-4 p-5">
              ...
              <Field label={t.message} error={error ?? create.error?.message}>
                <Textarea id="seller-inquiry-message" name="message" value={message} onChange={(event) => setMessage(event.target.value)} placeholder={t.placeholder} maxLength={4000} rows={5} />
              </Field>
              <div className="flex justify-end gap-2">
                <Button type="button" variant="ghost" onClick={() => setOpen(false)}>{t.cancel}</Button>
                <Button type="button" disabled={!productId || !message.trim() || sending} onClick={submit}>{sending ? t.sending : t.send}</Button>
              </div>
            </div>
          </section>
        </div>
