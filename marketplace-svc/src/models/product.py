...
from sqlalchemy import CheckConstraint, Computed, DateTime, Enum, Float, ForeignKey, Integer, String, Text, func
...
# Storefront search corpus: every locale's title + highlight, lower-cased and
# diacritic-stripped by the database (see alembic dx1a2b3c4d5e6). Kept in
# sync with the migration by hand; the GIN trigram index lives on this column.
PRODUCT_SEARCH_TEXT_SQL = (
...
class Product(Base):
    ...
    search_text: Mapped[str] = mapped_column(Text, Computed(PRODUCT_SEARCH_TEXT_SQL, persisted=True), nullable=False)
